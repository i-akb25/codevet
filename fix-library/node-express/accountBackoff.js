// Per-account exponential backoff — complements authLimiter (which is
// per-IP) with per-account tracking, since an attacker distributing
// attempts across many IPs would otherwise bypass IP-based limiting
// entirely. Uses exponential backoff (delay doubles each failed attempt)
// rather than a hard lockout, since hard lockouts let an attacker lock a
// real user out of their own account (a denial-of-service against your
// own users).
//
...
const BASE_DELAY_MS = parseInt(process.env.AUTH_BACKOFF_BASE_MS ?? "1000", 10); // 1s
const MAX_DELAY_MS = parseInt(process.env.AUTH_BACKOFF_MAX_MS ?? "300000", 10); // 5 min cap
const RESET_AFTER_MS = parseInt(process.env.AUTH_BACKOFF_RESET_MS ?? "3600000", 10); // 1 hour of no attempts resets the counter
...
function checkAccountBackoff(accountKey) {
  ...
  const requiredDelay = Math.min(
    BASE_DELAY_MS * 2 ** record.failCount,
    MAX_DELAY_MS,
  );
  ...
  if (elapsedSinceLastAttempt < requiredDelay) {
    return {
      blocked: true,
      retryAfterSeconds: Math.ceil((requiredDelay - elapsedSinceLastAttempt) / 1000),
    };
  }
